...
All the environments are registered in the `force_peg_rl` extension. They start
...
import force_peg_rl.tasks  # noqa: F401

# PLACEHOLDER: Extension template (do not remove this comment)
with contextlib.suppress(ImportError):
    import force_peg_rl.tasks_experimental  # noqa: F401
...
def _format_presets(preset_map: dict | None) -> str:
    ...
    Args:
        preset_map: Mapping of :class:`~force_peg_rl.utils.preset_target.PresetTarget`
    ...
    if preset_map is None:
        return "(unavailable)"
    from force_peg_rl.utils.preset_target import PresetTarget
...
def main():
    """Print all environments registered in `force_peg_rl` extension."""
    ...
    task_specs = [
        ...
        and spec.entry_point.startswith("force_peg_rl.")
        and (args_cli.keyword is None or args_cli.keyword in spec.id)
    ]

    if args_cli.show_presets:
        from force_peg_rl.utils.preset_cli import enumerate_task_presets
